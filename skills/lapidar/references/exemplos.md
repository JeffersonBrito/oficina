...
## Exemplo 3: corrigir (pequeno, com código)

### Antes

> o benchmark tá dando valor diferente do gabarito nos processos escaneados

### Depois

Tipo: **corrigir**. Premissa que mais pesa: "escaneado" = processo com páginas que passaram por transcrição de imagem.

```markdown
# Corrigir divergência do benchmark em processos escaneados

## Sintoma
O benchmark reporta valores diferentes do gabarito humano em processos cujas páginas passaram por
transcrição de imagem. Processos com texto nativo batem. Ainda não há caso específico anexado.

## Esperado
Processos escaneados atingem taxa de acerto equivalente aos nativos, ou a divergência tem causa identificada.

## Reprodução
1. Listar no gabarito os casos com páginas transcritas.
2. Rodar o benchmark só nesses casos e registrar, campo a campo, o que diverge.

## Hipóteses
1. A transcrição perde formatação numérica (`1.234,56` vira `1234 56`) e a etapa determinística não reconhece. Descartar: comparar o texto transcrito de uma página com valor conhecido contra o PDF.
2. As páginas transcritas ficam fora do contexto por estourar o orçamento de caracteres. Descartar: logar as páginas selecionadas em um caso escaneado.
3. O gabarito desses casos foi verificado em página diferente da transcrita. Descartar: conferir o campo de verificação nos casos divergentes.

## Restrições
- Não corrigir sem antes reproduzir com um caso concreto.
- Não alterar o gabarito para "fazer bater". Gabarito errado é reportado à parte.

## Insumos e referências
- `app/lawsuit/precatorio_v2/benchmark.py`, `transcription.py`, `triage.py`, `document_values.py`

## Critérios de aceite
- [ ] Existe um teste que reproduz a divergência em ao menos um caso escaneado e falhava antes.
- [ ] Esse teste passa depois.
- [ ] Nenhum caso nativo que batia antes deixou de bater.

## Como verificar
```bash
pytest tests/ -k precatorio_v2 -q
python -m app.lawsuit.precatorio_v2.benchmark
```

## Premissas
- "Escaneado" = ao menos uma página transcrita de imagem. Se for outro critério, ajustar o passo 1.

## Entrega esperada
Causa raiz em uma frase, diff, saída do benchmark antes e depois nos casos escaneados.
```
